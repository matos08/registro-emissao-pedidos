...
class UpdatePinturaById:
    """
    Create for pintura by id in the database
    """

    def __init__(self, id: int, payload: schemas.UpdatePinturaSchema):
        self._id = id
    ...
    async def execute(self) -> schemas.PinturaSchema:
        await self.validate()
        pintura = await self._repository.update(self._id, **self._payload.dict())
