...
class UpdateOrcamentoById:
    """
    Create for orcamento by id in the database
    """

    def __init__(self, id: int, payload: schemas.UpdateOrcamentoSchema):
        self._id = id
    ...
    async def execute(self) -> schemas.OrcamentoSchema:
        await self.validate()
        orcamento = await self._repository.update(self._id, **self._payload.dict())
