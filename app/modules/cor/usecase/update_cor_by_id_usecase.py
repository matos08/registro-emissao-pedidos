...
class UpdateCorById:
    """
    Create for a cor by id in the database
    """

    def __init__(self, id: int, payload: schemas.UpdateCorSchema):
        self._id = id
    ...
    async def execute(self) -> schemas.CorSchema:
        await self.validate()
        cor = await self._repository.update(self._id, **self._payload.dict())
